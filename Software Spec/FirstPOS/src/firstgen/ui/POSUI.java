...
//TODO Apply the state pattern to actions from the UI 
	...
	private Action addAction = new AbstractAction() {
		...
		public void actionPerformed(ActionEvent event) {
			...
			Product pd = register.addItem(id, quantity);
			if ( pd == null ) {
				displayMessage(ERROR_MESSAGE, "Product id "+id+" not found (try ids 10 - 20)");
			}
			// if OK then remove id and reset quantity
			if ( pd != null ) {
				String message = String.format("%s %s  %d @ %.2f", 
						pd.getProductId(), pd.getDescription(), quantity, pd.getPrice() );
